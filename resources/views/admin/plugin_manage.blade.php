...
@section('title', 'Kelola Plugins - Admin')
...
<div class="max-w-7xl w-full mx-auto space-y-12">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-zinc-200 dark:border-zinc-800 pb-8">
        <div>
            ...
            <h1 class="font-display text-4xl font-black uppercase tracking-tight mt-1">Kelola Plugins</h1>
            <p class="text-sm text-zinc-500">Sinkronisasi data plugin dengan halaman user</p>
        </div>
        <button onclick="document.getElementById('pluginModal').showModal()" class="w-full md:w-auto bg-black text-white dark:bg-white dark:text-black px-6 py-3 font-bold uppercase tracking-widest text-sm rounded-lg hover:scale-105 transition transform">
            + Tambah Plugin
        </button>
    </div>
    ...
    @if ($plugins->isEmpty())
        <div class="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-12 text-center">
            <p class="text-zinc-500 dark:text-zinc-400">Belum ada data plugin. Tambahkan item pertama.</p>
        </div>
    @else
                ...
                <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800/80 rounded-2xl p-6 sm:p-8 shadow-sm space-y-4">
                    <div class="flex items-start justify-between gap-3">
                        ...
                        <div class="flex flex-wrap justify-end gap-2">
                            ...
                            <form action="{{ route('admin.plugins.destroy', $item) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('Yakin ingin menghapus plugin ini?')" class="text-xs bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 px-3 py-1 rounded hover:bg-red-200 dark:hover:bg-red-900/50 transition">
                                    Hapus
                                </button>
                            </form>
                        </div>
...
<dialog id="pluginModal" class="backdrop:bg-black/50 backdrop:backdrop-blur-sm rounded-2xl p-4 sm:p-6 max-w-lg w-[calc(100%-2rem)] shadow-2xl">
    <div class="flex justify-between items-center mb-6">
        <h2 class="font-display text-2xl font-black uppercase tracking-tight" id="pluginModalTitle">Tambah Plugin</h2>
    ...
    <form id="pluginForm" method="POST" action="{{ route('admin.plugins.store') }}" class="space-y-4" enctype="multipart/form-data">
        ...
        <div>
            ...
            <input type="text" name="title" id="pluginTitle" class="w-full border border-zinc-200 dark:border-zinc-800 rounded-lg px-4 py-2 mt-2 bg-white dark:bg-zinc-900 focus:outline-none focus:border-black dark:focus:border-white transition" required>
        </div>

        <div>
            <label class="text-sm font-bold uppercase tracking-widest opacity-60">Harga</label>
            <input type="number" name="price" id="pluginPrice" min="0" step="0.01" class="w-full border border-zinc-200 dark:border-zinc-800 rounded-lg px-4 py-2 mt-2 bg-white dark:bg-zinc-900 focus:outline-none focus:border-black dark:focus:border-white transition" required>
        </div>

        <div>
            <label class="text-sm font-bold uppercase tracking-widest opacity-60">Deskripsi</label>
            <textarea name="description" id="pluginDescription" rows="4" class="w-full border border-zinc-200 dark:border-zinc-800 rounded-lg px-4 py-2 mt-2 bg-white dark:bg-zinc-900 focus:outline-none focus:border-black dark:focus:border-white transition resize-none" required></textarea>
        </div>
...
<script>
    function editPlugin(id, title, price, description) {
        ...
        document.getElementById('pluginModal').showModal();
    }

    document.getElementById('pluginModal').addEventListener('close', function() {
        document.getElementById('pluginModalTitle').textContent = 'Tambah Plugin';
        const form = document.getElementById('pluginForm');
        form.reset();
        form.action = '{{ route("admin.plugins.store") }}';
